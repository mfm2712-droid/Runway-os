...
export function BottomNav({
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 flex justify-center pb-[calc(14px+env(safe-area-inset-bottom))] px-4 pointer-events-none">
      <div className="relative pointer-events-auto">
        {showFab && (
          <button
            ...
            className="absolute z-20 left-1/2 -translate-x-1/2 -top-7 h-14 w-14 rounded-full bg-gradient-to-b from-cyan-400 to-cyan-500 flex items-center justify-center text-obsidian-950 shadow-[0_0_0_6px_rgba(5,5,10,1),0_8px_28px_-6px_rgba(25,217,160,0.7)] active:scale-90 transition-transform duration-150 [transition-timing-function:var(--ease-spring)]"
          >
        ...
        <div
          className="relative z-10 flex items-center gap-1 rounded-full glass-strong glass-inset px-2"
          style={{ height: navHeight }}
        >
          {leftTabs.map((t) => (
            <TabButton key={t.id} tab={t} active={active === t.id} onClick={() => changeTab(t.id)} />
          ))}
          {showFab && <span className="w-14" aria-hidden />}
          {rightTabs.map((t) => (
            <TabButton key={t.id} tab={t} active={active === t.id} onClick={() => changeTab(t.id)} />
          ))}
        </div>
  ...
  return (
    ...
    >
      <Icon
        width={20}
        height={20}
        style={active ? { filter: "drop-shadow(0 0 6px rgba(25,217,160,0.8))" } : undefined}
      />
