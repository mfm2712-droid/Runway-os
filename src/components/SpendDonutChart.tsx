...
export function SpendDonutChart({
  ...
  const [collapsed, setCollapsed] = useLocalStorage<boolean>("runway_spend_breakdown_collapsed", false);
  ...
  return (
    <GlassCard opaque className="p-6 space-y-5">
      <button
        ...
        aria-controls="spend-breakdown-content"
      ...
      >
        ...
        <span className="flex items-center gap-1.5 shrink-0">
          <span className="text-[10px] text-slate-500">{t("donut.tapForDetails")}</span>
          <ChevronDownIcon
            width={14}
            height={14}
            strokeWidth={2}
            className={`text-slate-500 transform transition-transform duration-200 ${collapsed ? "" : "rotate-180"}`}
          />
        </span>
      </button>

      {!collapsed && (
        <div id="spend-breakdown-content" className="space-y-5 animate-[floatIn_0.2s_var(--ease-spring)]">
